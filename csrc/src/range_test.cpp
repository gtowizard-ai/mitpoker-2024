...
TEST_F(RangeTest, TestUniformRandomRange) {
  ...
  ASSERT_NEAR(ranges::sum(range.range), 1.0, TOLERANCE);
}
...
TEST_F(RangeTest, TestUpdateOnNewBoardCards) {
  ...
  ASSERT_NEAR(ranges::sum(range.range), static_cast<double>(n_choose_k(49, 2)) / n_choose_k(52, 2),
              TOLERANCE);

  board_cards.push_back(Card("As").card());
  range.update_on_new_board_cards(game_, board_cards);

  ASSERT_NEAR(ranges::sum(range.range), static_cast<double>(n_choose_k(48, 2)) / n_choose_k(52, 2),
              TOLERANCE);

  board_cards.push_back(Card("2s").card());
  range.update_on_new_board_cards(game_, board_cards);

  ASSERT_NEAR(ranges::sum(range.range), static_cast<double>(n_choose_k(47, 2)) / n_choose_k(52, 2),
              TOLERANCE);
...
TEST_F(RangeTest, TestTo3CardsRangeWithBoardCards) {
  ...
  ASSERT_NEAR(two_cards_sum, static_cast<double>(n_choose_k(48, 2)) / n_choose_k(52, 2), TOLERANCE);
